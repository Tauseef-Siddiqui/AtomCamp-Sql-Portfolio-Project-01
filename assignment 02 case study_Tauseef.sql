...
SELECT region_id, COUNT(DISTINCT node_id) AS number_of_nodes
FROM customer_nodes GROUP BY region_id ORDER BY region_id;

-- 3 How many customers are allocated to each region?
SELECT region_id, COUNT(DISTINCT customer_id) AS number_of_customers
FROM customer_nodes GROUP BY region_id ORDER BY region_id;

-- 4. How many days on average are customers reallocated to a different node?
DROP TEMPORARY TABLE IF EXISTS ReallocationData;
CREATE TEMPORARY TABLE ReallocationData AS
SELECT 
    customer_id,
    region_id,
    node_id,
    ...
    start_date,
    previous_end_date,
    DATEDIFF(start_date, previous_end_date) AS days_between_reallocations
FROM (
    SELECT
        customer_id,
        region_id,
        node_id,
        LAG(node_id) OVER(PARTITION BY customer_id, region_id ORDER BY start_date) AS previous_node_id,
        start_date,
        LAG(end_date) OVER(PARTITION BY customer_id, region_id ORDER BY start_date) AS previous_end_date
    FROM
        customer_nodes
    ) AS subquery
WHERE
    previous_node_id IS NOT NULL
    AND DATEDIFF(start_date, previous_end_date) > 0;
    SELECT
    region_id,
    AVG(days_between_reallocations) AS average_days_between_reallocations
FROM
    ReallocationData
GROUP BY
    region_id;
...
DROP TEMPORARY TABLE IF EXISTS RegionCounts;
CREATE TEMPORARY TABLE RegionCounts AS
SELECT 
    region_id,
    COUNT(*) AS cnt
FROM 
...
DROP TEMPORARY TABLE IF EXISTS PercentileRanks;
CREATE TEMPORARY TABLE PercentileRanks AS
SELECT 
    RD.region_id,
    RD.days_between_reallocations,
    @row_number := IF(@region_id = RD.region_id, @row_number + 1, 1) AS rn,
    RC.cnt,
    @region_id := RD.region_id AS dummy
FROM 
    ReallocationData RD
JOIN 
    RegionCounts RC ON RD.region_id = RC.region_id
ORDER BY 
    RD.region_id, RD.days_between_reallocations;
    SELECT 
    region_id,
    AVG(CASE WHEN rn = CEIL(cnt * 0.5) THEN days_between_reallocations ELSE NULL END) AS median_approx,
    AVG(CASE WHEN rn = CEIL(cnt * 0.8) THEN days_between_reallocations ELSE NULL END) AS p80_approx,
    AVG(CASE WHEN rn = CEIL(cnt * 0.95) THEN days_between_reallocations ELSE NULL END) AS p95_approx,
    AVG(days_between_reallocations) AS average_days_between_reallocations
FROM 
    PercentileRanks
GROUP BY 
    ...
    SELECT 
    region_id,
    MAX(CASE WHEN rn <= CEIL(cnt * 0.5) THEN days_between_reallocations END) AS median_approx,
    MAX(CASE WHEN rn <= CEIL(cnt * 0.8) THEN days_between_reallocations END) AS p80_approx,
    MAX(CASE WHEN rn <= CEIL(cnt * 0.95) THEN days_between_reallocations END) AS p95_approx,
    AVG(days_between_reallocations) AS average_days_between_reallocations
FROM 
    PercentileRanks
GROUP BY 
...
GROUP BY 
    ...
    AVG(total_deposit_amount) AS average_total_deposit_amount
FROM (
    SELECT 
        customer_id,
...
WITH MonthlyTransactions AS (
    ...
    FROM 
        customer_transactions
    GROUP BY 
        customer_id, year, month, txn_type
),
Deposits AS (
    SELECT 
    ...
    FROM 
        MonthlyTransactions
    WHERE 
        txn_type = 'Deposit' AND transaction_count > 1
),
PurchasesOrWithdrawals AS (
    SELECT 
        customer_id, year, month
    FROM 
        MonthlyTransactions
    WHERE 
        txn_type IN ('Purchase', 'Withdrawal') AND transaction_count = 1
),
QualifiedCustomers AS (
    SELECT 
        D.customer_id, D.year, D.month
    FROM 
        Deposits D
    JOIN 
        PurchasesOrWithdrawals PW ON D.customer_id = PW.customer_id AND D.year = PW.year AND D.month = PW.month
)
SELECT 
    year, month, COUNT(DISTINCT customer_id) AS qualifying_customers
FROM 
    QualifiedCustomers
GROUP BY 
    year, month
...
CumulativeBalances AS (
    SELECT
        MB.customer_id,
        MB.month_end_date,
        SUM(MB.monthly_balance) OVER(PARTITION BY MB.customer_id ORDER BY MB.month_end_date) AS cumulative_balance
    FROM
        MonthlyBalances MB
)
SELECT
...
BalanceChanges AS (
    SELECT
        ...
        previous.closing_balance AS previous_balance,
        IFNULL(((current.closing_balance - previous.closing_balance) / previous.closing_balance) * 100, 0) AS balance_change_pct
    FROM
        MonthlyClosingBalances current
    LEFT JOIN
        MonthlyClosingBalances previous ON current.customer_id = previous.customer_id
        AND DATE_ADD(STR_TO_DATE(CONCAT(previous.year_month, '-01'), '%Y-%m-%d'), INTERVAL 1 MONTH) = STR_TO_DATE(CONCAT(current.year_month, '-01'), '%Y-%m-%d')
),
...
IncreasedCustomersCount AS (
    SELECT
        COUNT(DISTINCT customer_id) AS increased_customers
    FROM
        IncreasedBalances
)
SELECT
    (SELECT increased_customers FROM IncreasedCustomersCount) / (SELECT total_customers FROM TotalUniqueCustomers) * 100 AS percentage_of_customers_increased;
    
    SELECT 
    customer_id,
    LAST_DAY(DATE_SUB(txn_date, INTERVAL 1 MONTH)) AS allocation_month,
    SUM(txn_amount) AS closing_balance
FROM 
    customer_transactions
WHERE 
    txn_date <= LAST_DAY(DATE_SUB(txn_date, INTERVAL 1 MONTH))
GROUP BY 
    customer_id, allocation_month;
    ...
    WHERE 
        ct.txn_date BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE()
    GROUP BY 
        ct.customer_id, DATE(ct.txn_date)
),
average_balances AS (
    SELECT 
        db.customer_id,
        AVG(db.daily_balance) AS avg_balance
    FROM 
        daily_balances db
    GROUP BY 
        db.customer_id
...
RunningBalances AS (
    SELECT
        ...
        SUM(
            CASE
                ...
                WHEN ct.txn_type IN ('purchase', 'withdrawal') THEN -ct.txn_amount
                ELSE 0
            END
        ) OVER (
    ...
    FROM
        customer_transactions ct
    INNER JOIN LastTxnDate ltd ON ct.customer_id = ltd.customer_id AND ct.txn_date = ltd.last_txn_date
),
AvgLastBalance AS (
    SELECT
        customer_id,
        AVG(running_balance) AS avg_last_balance 
    FROM
        RunningBalances
    GROUP BY
        customer_id
...
LatestBalances AS (
    SELECT
        rb.customer_id,
        rb.txn_date AS last_txn_date,
        rb.running_balance
    FROM
        RunningBalances rb
    INNER JOIN LastTxnDate ltd ON rb.customer_id = ltd.customer_id AND rb.txn_date = ltd.last_txn_date
)
...
ORDER BY 
    ...
    SELECT
        DATE_FORMAT(MIN(txn_date), '%Y-%m-01') AS first_date,
        LAST_DAY(MAX(txn_date)) AS last_date
    FROM
        customer_transactions
    UNION ALL
    SELECT
...
MonthlyBalances AS (
    SELECT
        ct.customer_id,
        LAST_DAY(ct.txn_date) AS month_end_date,
        SUM(
            CASE
                WHEN ct.txn_type = 'deposit' THEN ct.txn_amount
                WHEN ct.txn_type IN ('withdrawal', 'purchase') THEN -ct.txn_amount
    ...
    FROM
        customer_transactions ct
    JOIN
        DateSeries ds ON ct.txn_date <= ds.last_date
),
EndOfMonthBalances AS (
    SELECT
        mb.customer_id,
        mb.month_end_date,
        MAX(mb.running_balance) AS balance_at_month_end
    FROM
        MonthlyBalances mb
    GROUP BY
        mb.customer_id, mb.month_end_date
)

SELECT
    eomb.customer_id,
    eomb.month_end_date,
    eomb.balance_at_month_end
FROM
    EndOfMonthBalances eomb
ORDER BY
    eomb.customer_id, eomb.month_end_date;
...
LastMonthBalances AS (
    SELECT
        ts.customer_id,
        ts.month_end,
        MAX(ts.running_balance) AS balance_at_month_end
    FROM
        TransactionSummary ts
    INNER JOIN LastMonthEnd lme ON ts.month_end = lme.last_month_end
    GROUP BY
        ts.customer_id, ts.month_end
)

SELECT
    lmb.customer_id,
    lmb.month_end AS end_of_last_month,
    lmb.balance_at_month_end AS running_balance_at_end_of_last_month
FROM
    LastMonthBalances lmb
ORDER BY
    lmb.customer_id;
    ...
    WITH DateRange AS (
    SELECT CURDATE() - INTERVAL 30 DAY AS start_date, CURDATE() AS end_date
),
DailyTransactions AS (
    SELECT
        ...
        dr.start_date,
        ct.txn_date,
        IF(ct.txn_type = 'deposit', ct.txn_amount, -ct.txn_amount) AS txn_amount
    FROM
        customer_transactions ct
        CROSS JOIN DateRange dr
    WHERE
        ct.txn_date BETWEEN dr.start_date AND dr.end_date
),
RunningBalances AS (
    SELECT
        dt.customer_id,
        dt.txn_date,
        SUM(dt.txn_amount) OVER (
            PARTITION BY dt.customer_id
...
AverageBalances AS (
    SELECT
        rb.customer_id,
        AVG(rb.daily_balance) AS avg_balance
    FROM
        RunningBalances rb
    GROUP BY
        rb.customer_id
...
RunningBalances AS (
    ...
    FROM
        customer_transactions ct
    JOIN LastTransactionDate ltd ON ct.customer_id = ltd.customer_id AND ct.txn_date <= ltd.last_date
),
BalancesOnLastDate AS (
    SELECT
        rb.customer_id,
        ltd.last_date,
        MAX(rb.running_balance) AS balance_on_last_transaction_date
    FROM
        RunningBalances rb
    JOIN LastTransactionDate ltd ON rb.customer_id = ltd.customer_id AND rb.txn_date = ltd.last_date
    GROUP BY
        rb.customer_id, ltd.last_date
)

SELECT
    customer_id,
    last_date AS last_transaction_date,
    balance_on_last_transaction_date
FROM
    BalancesOnLastDate
ORDER BY
    customer_id;
    ...
    -- Further balance is segregated in terms of High Priority and Low Priority Customers based on the balance in their respective account
...
SELECT
    customer_id,
    last_date AS last_transaction_date,
    CASE WHEN balance_on_last_transaction_date > 0 THEN balance_on_last_transaction_date END AS "Positive_Account_Balance_High_Priority",
    CASE WHEN balance_on_last_transaction_date < 0 THEN balance_on_last_transaction_date END AS "Negative_Account_Balance_Low_Priority"
FROM
    BalancesOnLastDate
ORDER BY
    customer_id;
